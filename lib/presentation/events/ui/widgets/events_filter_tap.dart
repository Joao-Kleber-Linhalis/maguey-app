...
void onEventFilterTap(
  BuildContext context,
//Function() onTap
) {
  CustomBottomSheet.openBottomSheet(context,
      child: const EvntsListViewFilterWIdget(
          //   onTap: onTap
          ));
}

class EvntsListViewFilterWIdget extends StatefulWidget {
  // final Function() onTap;
  const EvntsListViewFilterWIdget({
    super.key,
    //   required this.onTap,
  });

  @override
  State<EvntsListViewFilterWIdget> createState() =>
      _EvntsListViewFilterWIdgetState();
}

class _EvntsListViewFilterWIdgetState extends State<EvntsListViewFilterWIdget> {
  bool isChecked = false;
  //double distance = 1;
  TextEditingController eventController = TextEditingController();

  // List<FlavorModel> filters = [
  //   FlavorModel(name: "Retail", value: 1),
  //   FlavorModel(name: "Wholesale", value: 2),
  //   FlavorModel(name: "Shop", value: 3),
  // ];

  @override
  void initState() {
    eventController.text = eventStore.state.recentSearch!;
      ...
      builder: (context, store) => Column(
        ...
        children: [
          MyTextFormField(
            ...
            onTap: () async {
              ...
              if (store.state.placeSearchResult != null) {
                ...
                setState(() {
                  eventController.text = store.state.recentSearch!;
                });
              }
              FocusScope.of(context).unfocus();
            },
          ...
          MyLoaderElvButton(
            ...
            onPressed: () {
              if (store.state.placeSearchResult != null) {
                eventStore.dispatch(SearchEventByFiltersAction(
                    //  shopTypes: shopStore.state.shopSearchFilters!,
                    ));
              } else {
                eventStore.dispatch(SearchEventWithoutAdressAction());
              }
